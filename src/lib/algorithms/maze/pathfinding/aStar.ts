...
export const aStar = (grid: GridType, startTile: TileType, endTile: TileType) => {
  ...
  functionCost[base.row][base.col] = base.distance + heuristicCost[base.row][base.col]; // Calculate the initial function cost for the start tile
      ...
      for (let i = 0; i < neighbors.length; i += 1) {
        ...
        if (distanceToNeighbor < neighbors[i].distance) {
          ...
          functionCost[neighbors[i].row][neighbors[i].col] =
            neighbors[i].distance + heuristicCost[neighbors[i].row][neighbors[i].col]; // Update the function cost for the neighbor
