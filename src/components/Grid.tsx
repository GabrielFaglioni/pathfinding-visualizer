  ...
  return (
    <div
      className={twMerge(
        ...
        // Control Grid height
        `lg:min-h-[${MAX_ROWS * 17}px]  md:min-h-[${MAX_ROWS * 15}px] xs:min-h-[${MAX_ROWS * 8}px] min-h-[${
          MAX_ROWS * 7
        }px]`,
      ...
      {grid.map(
        (row, rowIndex): JSX.Element => (
          <div key={rowIndex} className="flex">
            {row.map((tile, tileIndex): JSX.Element => {
              ...
              return (
                <Tile
                  ...
                  isWall={isWall}
                  handleMouseDown={(): void => handleMouseDown(row, col)}
                  handleMouseUp={(): void => handleMouseUp(row, col)}
                  handleMouseEnter={(): void => handleMouseEnter(row, col)}
                />
              );
            })}
          </div>
