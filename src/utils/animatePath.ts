...
export const animatePath = (
  ...
  for (let i = 0; i < traversedTiles.length; i++) {
    ...
    }, SLEEP_TIME * i * SPEEDS.find((s): boolean => s.value === speed)!.value); // Calculate delay based on speed
  }
  ...
  setTimeout((): void => {
    for (let i = 0; i < path.length; i++) {
      ...
      }, EXTENDED_SLEEP_TIME * i * SPEEDS.find((s): boolean => s.value === speed)!.value); // Calculate delay based on speed
    }
  }, SLEEP_TIME * traversedTiles.length * SPEEDS.find((s): boolean => s.value === speed)!.value); // Calculate delay based on the total traversal time
};
